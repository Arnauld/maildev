...
function handleDataStream (stream, session, callback) {
  ...
  const messageHandler = mailServer.store.newMessageHandler(id, {
    from: session.envelope.mailFrom,
    to: session.envelope.rcptTo,
    host: session.hostNameAppearsAs,
    remoteAddress: session.remoteAddress
  })
  const emlStream = messageHandler.emlWriteStream()
  emlStream.on('open', function () {
    ...
    parseStream.on('headers', headers => {
      messageHandler.headers({
        ...
        time: headers.date || new Date()
      })
    })

    parseStream.on('data', data => {
      if(data.type === 'attachment') {
        const attachmentStream = messageHandler.newAttachmentWriteStream({
        ...
        data.content.pipe(attachmentStream)
        data.content.on('end', () => {
    ...
    parseStream.on('end', messageHandler.persist)
